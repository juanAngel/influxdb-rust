...
impl Client2 {
    ...
    ///
    /// let _client = Client2::new("http://localhost:8086", "test");
    /// ```
    #[must_use = "Creating a client is pointless unless you use it"]
    pub fn new<S1, S2>(url: S1, org: S2) -> Self
    where
        S1: Into<String>,
        S2: Into<String>,
    {
        ...
        headers.insert(header::CONTENT_TYPE, "application/vnd.flux".parse().unwrap());
    ...
    /// Returns the name of the database the client is using
    pub fn database_org(&self) -> &str {
        // safe to unwrap: we always set the database name in `Self::new`
        self.parameters.get("org").unwrap()
    }
    ...
    ///
    /// ```rust,no_run
    /// use influxdb::{Client2, Query, Timestamp};
    /// use influxdb::InfluxDbWriteable;
    /// use std::time::{SystemTime, UNIX_EPOCH};
    ///
    /// # #[async_std::main]
